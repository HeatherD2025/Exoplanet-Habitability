...
function poolConfig(connectionString: string): pg.PoolConfig {
  ...
  // Strip query parameters to avoid driver conflicts
...
function resolveTargets(mode: SeedMode): SeedTarget[] {
  ...
  if (mode === "supabase") {
    if (!supabaseUrl) {
      throw new Error(
        "Set DIRECT_URL or DATABASE_URL in .env for Supabase seeding.",
      );
    }
  ...
  if (supabaseUrl) {
    const alreadyAdded = targets.some(
      (t) => t.connectionString === supabaseUrl,
    );
    if (!alreadyAdded) {
...
async function fetchNasaPlanets(): Promise<NasaPlanetRow[]> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch source data: ${response.status} ${response.statusText}`,
    );
  }
...
async function seedDatabase(
  ...
  const positiveOrNull = (value: number | null | undefined): number | null =>
    typeof value === "number" && Number.isFinite(value) && value > 0
      ? value
      : null;
  const positiveAtScale6OrNull = (
    value: number | null | undefined,
  ): number | null =>
    typeof value === "number" && Number.isFinite(value) && value >= 0.000001
      ? value
      : null;
  const finiteOrNull = (value: number | null | undefined): number | null =>
    typeof value === "number" && Number.isFinite(value) ? value : null;
  const nonNegativeIntOrNull = (
    value: number | null | undefined,
  ): number | null => {
  ...
  // DB batching chunk size (keeps transaction payloads optimal for Supabase)
  const SEED_BATCH_SIZE = 100;

  try {
    for (let i = 0; i < data.length; i += SEED_BATCH_SIZE) {
      const chunk = data.slice(i, i + SEED_BATCH_SIZE);
      ...
      await prisma.$transaction(
        async (tx) => {
          for (const planet of chunk) {
            ...
            let mass = positiveOrNull(
              planet.pl_bmassj ? planet.pl_bmassj * 317.8 : null,
            );
            ...
            const stellarLuminosityLinear =
              typeof planet.st_lum === "number" &&
              Number.isFinite(planet.st_lum)
                ? Math.pow(10, planet.st_lum)
                : null;
            const stellarFlux =
              stellarLuminosityLinear && orbitalDistance
                ? positiveAtScale6OrNull(
                    stellarLuminosityLinear / Math.pow(orbitalDistance, 2),
                  )
                : null;
            const equilibriumTemperatureKelvin = finiteOrNull(planet.pl_eqt);
            const transmissionSpectroscopyCount = nonNegativeIntOrNull(
              planet.pl_ntranspec,
            );
            const eclipseSpectroscopyCount = nonNegativeIntOrNull(
              planet.pl_nespec,
            );
            const directImagingSpectroscopyCount = nonNegativeIntOrNull(
              planet.pl_ndispec,
            );
      ...
      console.log(
        `Seeded batch ${Math.min(i + SEED_BATCH_SIZE, data.length)} / ${data.length} planets...`,
      );
    }
...
async function main() {
  ...
  if (mode !== "all" && mode !== "local" && mode !== "supabase") {
    throw new Error(
      `Invalid SEED_TARGET "${mode}". Use all, local, or supabase.`,
    );
  }
